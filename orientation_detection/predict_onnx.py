...
def rotate_img(original_path, angle, output_path):
    # Open original image
    img = Image.open(original_path)

    # Rotate according to class
    if angle == 1:        # 90° Clockwise
        rotated_img = img.rotate(-90, expand=True)
    elif angle == 2:      # 180°
        rotated_img = img.rotate(180, expand=True)
    elif angle == 3:      # 90° Counter-Clockwise
        rotated_img = img.rotate(90, expand=True)
    else:                 # 0° = no rotation
    ...
    # Save rotated copy
    output_path = os.path.join(output_path, os.path.basename(original_path))
    rotated_img.save(output_path)

    print(f"Saved rotated image to {output_path}")

def predict_single_image_onnx(ort_session, image_path, image_transforms,output_path):
    ...
    rotate_img(image_path, predicted_class, output_path=output_path)
...
def main(args_list=None):  # <--- 1. Wrap it in a function with the 'valve'
    ...
    # 2. Tell the parser to use our list if we provide one
